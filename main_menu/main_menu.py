                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    if self.btn[0] <= x <= self.btn[0] + self.btn[2]:
                        if self.btn[1] <= y <= self.btn[1] + self.btn[3]:
                            game = Game(self.win,True, [], None, False)
                            game.run()
                            del game
                    if self.ai_b[0] <= x <= self.ai_b[0] + self.ai_b[2]:
                        if self.ai_b[1] <= y <= self.ai_b[1] + self.ai_b[3]:
                            if PARALLEL_MODE:
                                ...
                                gaAlgo = SerialGeneticAlgorithm(self.win,VISUAL_MODE, READ_FILE, SAVE_TO_DISK, PRINT_GRAPHS, COLLECT_WHOLE_GAME_DATA, COLLECT_INNER_GAME_DATA)
                            gaAlgo.run()
                            # AIplay(self.win)
                            # AIplay.g_run()
            self.draw()
