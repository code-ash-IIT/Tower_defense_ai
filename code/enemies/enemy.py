...
class Enemy:
    ...
    def move(self, ticks):
        if self.frozen:
            ...
            self.animationCount+=1
            if(self.animationCount>=len(self.images)*self.animationSpeed):
                self.animationCount=0

            if(self.pathIndex<len(self.path)):
                if(self.x<(self.path[self.pathIndex])[0]):
                    self.x+=self.velocity
                elif(self.x>(self.path[self.pathIndex])[0]):
                    self.x-=self.velocity
                if(self.y<(self.path[self.pathIndex])[1]):
                    self.y+=self.velocity
                elif(self.y>(self.path[self.pathIndex])[1]):
                    self.y-=self.velocity
                z=(self.path[self.pathIndex][0]-self.x, self.path[self.pathIndex][1]-self.y)

                # if(z[0]<=0 and not(self.flipped)):
                #     self.flipped=True
                #     for count, img in enumerate(self.images):
                #         self.images[count]=pygame.transform.flip(img, True, False)
                # elif(z[0]>0 and self.flipped):
                #     self.flipped=False
                #     for count, img in enumerate(self.images):
                #         self.images[count]=pygame.transform.flip(img, True, False)
                if(abs(z[0])//self.velocity, abs(z[1])//self.velocity)==(0,0):
                    self.pathIndex+=1
            else:
                self.completed=True
