...
class Game:
    ...
    def __init__(self, visualMode, towers, gameRecord, collectInnerGameData, deepQagent):
        ...
        self.addedSpeed   = 0
        self.towers       = towers
        self.towers.append(City((739, 362)))
        self.pos = []
    ...
    def enemiesMove(self, ticks):                               #check
        for enemy in self.enemies:
            enemy.move(ticks)
    ...
    def handleEvents(self):
        ...
        for event in pygame.event.get():
            ...
            # to find the position where mouse is clicked
            # if event.type == pygame.MOUSEBUTTONUP:
            #     self.pos.append(mousePosition)
            #     print(self.pos)
            # for pos in self.pos:
            #     pygame.draw.circle(self.bg,(255,0,0),pos,4)
    ...
    def removeEnemies(self):                                         #CHECK
        for enemy in self.enemies:
            ...
            if (enemy.completed or enemy.health <= 0):
    ...
    def displayTextUI(self, win, ):
        ...
        self.displayText(numEnemiesText, numEnemiesPosition, self.uiFont, BLACK)

        self.displayText("Level: " + str(self.level), ((numEnemiesPosition[0] , numEnemiesPosition[1] - 25)), self.uiFont, BLACK)

        self.displayText("Dead: " + str(self.totalEnemiesKilled), ((numEnemiesPosition[0] , numEnemiesPosition[1] - 50)), self.uiFont, BLACK)
        self.displayText("Towers: " + str(len(self.towers)), (numEnemiesPosition[0], numEnemiesPosition[1] - 75), self.uiFont, BLACK)

        #Health
        healthText = "Health: " + str(int(self.health))
        healthPosition = (self.coinPosition[0], self.coinPosition[1] + 60)
        self.displayText(healthText, healthPosition, self.uiFont, self.getHealthColor())

        #Score
        self.displayText("Score: " + str(self.score), (self.coinPosition[0], self.coinPosition[1] + 30), self.uiFont, (255,215,0))
